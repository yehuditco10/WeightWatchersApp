...
create or replace function pg_temp.create_saga_table_MeasurePolicy(tablePrefix varchar, schema varchar)
    returns integer as
    $body$
    declare
...
/* WriteCreateIndex measureId */
        ...
        script = 'create unique index if not exists "' || tablePrefix || '_i_3452BBB5766DB5B0E3C2C4876CCB11EDB30C9670" on "' || schema || '"."' || tableNameNonQuoted || '" using btree ("Correlation_measureId" asc);';
        execute script;
/* PurgeObsoleteIndex */
...
select pg_temp.create_saga_table_MeasurePolicy(@tablePrefix, @schema);
